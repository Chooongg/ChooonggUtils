...
fun Context.attrText(@AttrRes id: Int): CharSequence? =
    obtainStyledAttributes(intArrayOf(id)).use { it.getText(0) }

fun Context.attrString(@AttrRes id: Int) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getString(0) }

fun Context.attrString(@AttrRes id: Int, vararg format: Any?) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getString(0)?.format(*format) }

fun Context.attrBoolean(@AttrRes id: Int, defValue: Boolean) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getBoolean(0, defValue) }

fun Context.attrInt(@AttrRes id: Int, defValue: Int) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getInt(0, defValue) }

fun Context.attrFloat(@AttrRes id: Int, defValue: Float) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getFloat(0, defValue) }

fun Context.attrColor(@AttrRes id: Int, @ColorInt defValue: Int = Color.GRAY) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getColor(0, defValue) }

fun Context.attrColorStateList(@AttrRes id: Int) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getColorStateList(0) }

fun Context.attrInteger(@AttrRes id: Int, defValue: Int) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getInteger(0, defValue) }

fun Context.attrDimension(@AttrRes id: Int, defValue: Float) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getDimension(0, defValue) }

fun Context.attrDimensionPixelOffset(@AttrRes id: Int, defValue: Int) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getDimensionPixelOffset(0, defValue) }

fun Context.attrDimensionPixelSize(@AttrRes id: Int, defValue: Int) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getDimensionPixelSize(0, defValue) }

fun Context.attrResourcesId(@AttrRes id: Int, defValue: Int) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getResourceId(0, defValue) }

fun Context.attrDrawable(@AttrRes id: Int) =
    obtainStyledAttributes(intArrayOf(id)).use { it.getDrawable(0) }
...
fun View.attrString(@AttrRes id: Int, vararg format: Any?) =
    context.attrString(id, *format)
